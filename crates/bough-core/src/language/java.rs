    ...
    fn check_node(
        ...
        let result = match node.kind() {
            ...
            "if_statement" | "while_statement" => {
                let paren_expr = node.child_by_field_name("condition")?;
                let inner = paren_expr.named_child(0)?;
                Some((
        ...
        if let Some((ref kind, ref span, _)) = result {
            trace!(node_kind = node.kind(), mutant_kind = ?kind, start_byte = span.start().byte(), "java: matched node");
        }
        result
    }
...
mod tests {
    fn dump_tree(src: &str) {
        let lang: arborium_tree_sitter::Language = arborium_java::language().into();
