...
impl LanguageDriver for CDriver {
    ...
    fn check_node(
        ...
        if let Some((ref kind, ref span, _)) = result {
            trace!(node_kind = node.kind(), mutant_kind = ?kind, start_byte = span.start().byte(), "c: matched node");
        }
        result
    }
...
mod tests {
    fn dump_tree(src: &str) {
        let lang: arborium_tree_sitter::Language = arborium_c::language().into();
