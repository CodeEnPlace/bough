...
fn merge_cli_overrides_config() {
    let config = PartialSession {
        language: Some(bough_core::config::LanguageId::Javascript),
        parallelism: Some(4),
        ..Default::default()
    };
    ...
    assert_eq!(merged.language, Some(bough_core::config::LanguageId::Javascript));
}
...
"#;
    ...
    assert_eq!(partial.language, Some(bough_core::config::LanguageId::Javascript));
