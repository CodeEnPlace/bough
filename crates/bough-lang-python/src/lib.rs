...
use bough_core::mutant::{AssignMutationKind, BinaryOpMutationKind, MutantKind, Span, span_from_node};
use tracing::trace;
...
impl LanguageDriver for PythonDriver {
        ...
        let result = match node.kind() {
            ...
            "string" => {
                ...
                let kind = if text == "\"\"" || text == "''" {
                    bough_core::mutant::LiteralKind::EmptyString
                } else {
                    bough_core::mutant::LiteralKind::String
                };
                let span = span_from_node(node);
            ...
            "integer" | "float" => {
                let span = span_from_node(node);
                Some((
                    MutantKind::Literal(bough_core::mutant::LiteralKind::Number),
                    span.clone(),
                    span,
                ))
            }
            "assert_statement" => {
                let span = span_from_node(node);
                Some((MutantKind::Assert, span.clone(), span))
            }
            "not_operator" => {
            ...
            "true" => {
                let span = span_from_node(node);
                Some((
                    MutantKind::Literal(bough_core::mutant::LiteralKind::BoolTrue),
                    span.clone(),
                    span,
                ))
            }
            "false" => {
                let span = span_from_node(node);
                Some((
                    MutantKind::Literal(bough_core::mutant::LiteralKind::BoolFalse),
                    span.clone(),
                    span,
                ))
            }
    ...
    fn substitutions(&self, kind: &MutantKind) -> Vec<String> {
        match kind {
            ...
            MutantKind::Literal(bough_core::mutant::LiteralKind::BoolTrue) => vec!["False".into()],
            MutantKind::Literal(bough_core::mutant::LiteralKind::BoolFalse) => vec!["True".into()],
            MutantKind::Assert => vec!["pass".into()],
            MutantKind::UnaryNot => vec!["".into()],
            MutantKind::Literal(bough_core::mutant::LiteralKind::String) => vec!["\"\"".into()],
            MutantKind::Literal(bough_core::mutant::LiteralKind::EmptyString) => {
                vec!["\"bough\"".into()]
            }
            MutantKind::Literal(bough_core::mutant::LiteralKind::Number) => vec![
                "0".into(),
                "1".into(),
                "-1".into(),
                "float('inf')".into(),
